...
secure way of contact to you. Unprotected email is not enough, even if someone tries
to contact you for the first time.

The Encrypted Contact plugin is the most secure way to offer secure messages to your website
users and it even enables the website owner to  read (i.e. to decrypt) protected messages
...
the website owner can stay in touch with the visitors, even if he is on the road, because
all he needs is stored on the server. Total device independence is assured. Neither
...
removed the powered by Kerry Linux link from all these scripts as a default.
If you wish to display this link on your website, you can replace the default files with
the original files you'll find in the directory `kerrylinux`.

All files from the Web Encryption Extension are code-signed by Kerry Linux, so the signature
verification will fail on the default files because of the missing link, but it will
validate correctly on the files in the directory `kerrylinux`. Check the difference, you'll
